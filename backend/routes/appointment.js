...
// Patient marks appointment as completed
router.put(
  "/complete/:id",
  authenticate,
  requireRole("patient"),
  async (req, res) => {
    try {
      ...
      res.ok(appointment, "Appointment marked as completed");
    } catch (error) {
      console.error("Patient complete appointment error", error);
      res.serverError("Failed to mark appointment completed", [error.message]);
    }
  }
);
