...
router.put('/appointments/:appointmentId/prescription',
    authenticate,
    requireRole('doctor'),
    [
        body('prescription').notEmpty().isString()
    ],
    validate,
    async (req, res) => {
        try {
            ...
            if (appointment.doctorId.toString() !== doctorId.toString()) {
                return res.forbidden('This appointment does not belong to you');
            }

            appointment.prescription = prescription;
            await appointment.save();

            res.ok(appointment, 'Prescription added successfully');
        } catch (error) {
            res.serverError('Failed to add prescription', [error.message]);
        }
    }
);
